...
import { validateDraftBranchName } from "../../_utils/security.js";
...
export async function onRequestGet({ env, request }) {
  ...
  let branch;
  try {
    branch = validateDraftBranchName(url.searchParams.get("branch"));
  } catch (error) {
    return Response.json({ error: error instanceof Error ? error.message : "branch is required" }, { status: 400 });
  }
  try {
